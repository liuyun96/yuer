...
public class SubjectsService {
	...
	public void delete(String ids) {
		...
		ht.getSession()
				.createQuery("delete from Subjects where id in (" + ids + ")")
				.executeUpdate();
	}

	@SuppressWarnings("unchecked")
	public Collection<Subjects> findByColumnId(Integer columnId) {
		return ht.query(
				" from Subjects where status =1 and columns.id = " + columnId +" order by position ")
				.list();
	}
