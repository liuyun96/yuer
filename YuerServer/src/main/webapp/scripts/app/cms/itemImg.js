function createItemImgPanel(apptitle) {
	// 分页大小。
	var pagesize = 25;
	// 定义编辑表单的界面
	var itemImgFormConfig = {
		frame : true,
		labelWidth : 70,
		autoHeight : true,
		id : 'itemImgform',
		...
		buttons : [{
			text : '保&nbsp;&nbsp;&nbsp;存',
			handler : function() {
				if (Ext.getCmp('itemImgform').getForm().isValid()) {
					Ext.getCmp('itemImgform').getForm().submit({
						url : webRoot + '/itemImg/save',
						waitMsg : '正在保存...',
						method : 'post',
						success : function(form, action) {
							Ext.MessageBox.alert('提示', '保存成功');
							Ext.getCmp('itemImgformwindow').close();
							store.load();
						},
	...
	var panel = new Ext.Panel({
		...
		tbar : [{
					...
					disabled : true,
					handler : function() {
						createItemImgForm('编辑商品图片', sm.getSelected().data.id);
					}
				}, {
					text : '删除',
					ref : '../removeButton',
	...
	function createItemImgForm(title, id) {
		var itemImgForm = new Ext.form.FormPanel(itemImgFormConfig);
		...
		if (id != undefined) {
			itemImgForm.getForm().load({
						clientValidation : false,
						waitMsg : '加载中...',
						url : webRoot + '/itemImg/load/' + id,
						method : 'GET'
					});
		}
		window.show();
