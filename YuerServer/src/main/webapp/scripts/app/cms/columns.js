function createColumnsPanel(apptitle){
	...
	var columnsFormConfig = {
		...
		//name是form提交是用到的名字,id:为dom所用,dataIndex:为映射数据所用。
				{name:'columns.id',id:'id',xtype:'hidden'},
				{name:'columns.id',id:'id',xtype:'hidden'},
				{name:'columns.status',value:1,id:'status',xtype:'checkbox',fieldLabel:'状态'}
		],
		buttons:[
			{
				text: '保&nbsp;&nbsp;&nbsp;存',
				handler:function(){
					if(Ext.getCmp('columnsform').getForm().isValid()){
						Ext.getCmp('columnsform').getForm().submit({
	...
	var panel = new Ext.Panel({
		...
		tbar:[
			...
			{
				text:'编辑',ref: '../editButton',tooltip:'编辑选中的栏目表',iconCls:'edit-icon',disabled:true,
				handler:function(){
					createColumnsForm('编辑栏目表',sm.getSelected().data.id);
				}
			},			
			{
				text:'删除',ref: '../removeButton',tooltip:'删除选中的栏目表',iconCls:'delete-icon',disabled:true,
				handler:function(){
