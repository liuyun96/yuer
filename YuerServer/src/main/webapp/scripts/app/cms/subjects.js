function createSubjectsPanel(apptitle){
	//分页大小。
	var pagesize=25;
	//定义编辑表单的界面
	var subjectsFormConfig = {
		...
		//name是form提交是用到的名字,id:为dom所用,dataIndex:为映射数据所用。
				{name:'subjects.id',id:'id',xtype:'hidden'},
				{name:'subjects.subjectName',id:'subjectName',fieldLabel:'专题名称'},
				{name:'subjects.status',value:1,id:'status',xtype:'checkbox',fieldLabel:'状态'}
		],
		buttons:[
			{
				text: '保&nbsp;&nbsp;&nbsp;存',
				handler:function(){
					if(Ext.getCmp('subjectsform').getForm().isValid()){
						Ext.getCmp('subjectsform').getForm().submit({
							url:webRoot+'/subjects/save',
							waitMsg:'正在保存...',
							method:'post',
	...
	var grid= new Ext.grid.GridPanel({
		...
		cm:new Ext.grid.ColumnModel({
			columns:[
				sm,
				{id:'subjectName',header:'专题名称',width:80,sortable:true,dataIndex:'subjectName'},
	...
	var panel = new Ext.Panel({
		...
		tbar:[
			...
			{
				text:'编辑',ref: '../editButton',tooltip:'编辑选中的专题表',iconCls:'edit-icon',disabled:true,
				handler:function(){
					createSubjectsForm('编辑专题表',sm.getSelected().data.id);
				}
			},			
			{
				text:'删除',ref: '../removeButton',tooltip:'删除选中的专题表',iconCls:'delete-icon',disabled:true,
				handler:function(){
