function createCouponAuditPanel(apptitle) {
	// 分页大小。
	var pagesize = 25;
	// 定义编辑表单的界面
	...
	var store = new Ext.data.JsonStore({
		...
		proxy : new Ext.data.HttpProxy({
			method : 'post',
			url : webRoot + '/coupon/list?coupon.type=1'
		})
	});
	...
	var panel = new Ext.Panel({
		...
		tbar : [
				...
				},
				{
					...
					disabled : true,
					handler : function() {
						Ext.MessageBox.confirm('审核确认', '你确定允许所选中的优惠劵通过审核吗？',
								function(id) {
									if (id == 'yes') {
										var ids = '';
										// 审核优惠劵。
				...
				},
				{
					...
					disabled : true,
					handler : function() {
						Ext.MessageBox.confirm('驳回确认', '你确定要驳回所选中的优惠劵吗？',
								function(id) {
									if (id == 'yes') {
										var ids = '';
										// 审核优惠劵。
